...
async function handleClientRegistrationConfirmation(socket, data, cb) {
    try {
        ...
        const users_private_fields = Object.fromEntries(Object.entries(preview_data).filter(([key]) => constant.database.users_private.column_names.includes(key)));
        const users_public_fields = Object.fromEntries(Object.entries(preview_data).filter(([key]) => constant.database.users_public.column_names.includes(key)));
